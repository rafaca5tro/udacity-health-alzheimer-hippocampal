...
def mpl_image_grid(images: torch.Tensor) -> plt.Figure:
    """
    Create an image grid from an array of images. Show up to 16 images in one figure.

    Arguments:
        images {Torch tensor} -- NxWxH array of images
...
def med_reshape(image: np.ndarray, new_shape: tuple) -> np.ndarray:
    """
    Reshape 3D data to new dimension padding with zeros
